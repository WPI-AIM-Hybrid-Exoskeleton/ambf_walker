...
// Generated 22-Aug-2021 17:11:16
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"simple_PD\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(simple_PD_M, (NULL));
        simple_PD_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(simple_PD_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(simple_PD_M)
                == (NULL)) || rtmGetStopRequested(simple_PD_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(simple_PD_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
